...
void engine::moveBalloon() //Don't use this yet
{
    ...
    //Default class retrieval
    jclass clazz = jni->GetObjectClass( _app->activity->clazz );
    jmethodID methodID = jni->GetMethodID(clazz, "moveBalloon", "()V" );
    jni->CallVoidMethod( _app->activity->clazz, methodID);
...
void engine::updateTime()
{
    ...
    //Default class retrieval
    jclass clazz = jni->GetObjectClass( _app->activity->clazz );
    jmethodID methodID = jni->GetMethodID(clazz, "updateTime", "()V" );
    jni->CallVoidMethod( _app->activity->clazz, methodID);
    ...
    if( id == LOOPER_ID_USER )
    {
        if (_accelSensor != NULL)
        {
                    ...
                    &event, 1) > 0)
            {
                ...
                Vy = event.acceleration.y;
                Vz = event.acceleration.z;
